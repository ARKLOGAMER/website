<template>
	<div class="flex mx-auto w-full max-w-7xl">
		<Sidebar v-if="docs" />
...
const watchRoute = async () => {
	...
	if (
		// @ts-ignore
		docs.value?.id !== (source.value?.id ?? sources[route.params.source].id) ||
		docs.value?.tag !== route.params.tag
	) {
		// @ts-ignore
		await fetchDocs(sources[route.params.source] ?? MainSource, tag.value ?? route.params.tag);
	}
